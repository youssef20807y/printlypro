<?php
/**
 * صفحة إضافة مقال جديد للوحة تحكم مطبعة برنتلي
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $check_slug = $db->prepare("SELECT blog_id FROM blog WHERE slug = ?");
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            $image_name = upload_blog_image($_FILES['image']);
            if (!$image_name) {
                throw new Exception('حدث خطأ أثناء تحميل الصورة');
            }
        }
        
        // إدخال البيانات في قاعدة البيانات
        $stmt = $db->prepare("
            INSERT INTO blog (title, slug, content, image, category, tags, author_id, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
        ");
        
        $stmt->execute([$title, $slug, $content, $image_name, $category, $tags, $author_id, $status]);
        
        $success_message = 'تمت إضافة المقال بنجاح';
        
        // إعادة التوجيه إلى صفحة المدونة بعد الإضافة
...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">إضافة مقال جديد</h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="card-body">
                    <form action="blog-add.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="tags" name="tags" placeholder="أدخل الوسوم مفصولة بفواصل">
